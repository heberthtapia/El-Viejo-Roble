		...
		***************************************************************/
        ...
        form.find('input:checkbox').each(function () {
            ...
            input.click(function () {
				...
				var dato = $('#formGeneral').find('input#fecha').val();
				
				if(dato == ""){
						jQuery('#fecha').validationEngine('validate');
				}else{
			...
			var IdealSelect = function(){ // Ideal Select menu
				...
				this.setWidth = function(){
					...
					/*	$(this).css('fontWeight', 'bold');
						...
						$(this).css('fontWeight', 'bold');		
						var currentWidth = $(this).text().length*5;	
						if (currentWidth > maxWidth) { maxWidth = currentWidth; }
						$(this).css('fontWeight', 'normal');
					});									
					//Set select width	
					//var w = $('select').width();
					/*$('#form_general ul').find('li').each(function(){	
					$(this).css('fontWeight', 'bold');		
					var currentWidth = $(this).text().length*5;	
						if (currentWidth > maxWidth) { maxWidth = currentWidth; }
						$(this).css('fontWeight', 'normal');
					...
					var headerHeight_v= ((headerHeight-(arrowHeight/2))/2);
						if (headerHeight_v<0){
							headerHeight_v=11.5;
							}				
					this.headerArrow.css({
						'left': (maxWidth) + 'px',
						'top': headerHeight_v+ 'px'
					});						
